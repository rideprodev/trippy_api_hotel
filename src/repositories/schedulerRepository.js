...
export default {
  ...
  async checkbiddingforbooking(biddingData) {
    ...
    if (filterRateData.length > 0) {
      ...
      reavalidateResponse = await grnRepository.revalidate(revalidate_request);
      if (reavalidateResponse?.data?.hotel?.rate?.rate_type === "bookable") {
        ...
        //  set the members for the new booking
        let members = [];
        if (Bidding.bookingGroupData.isUserTravelled === "true") {
          const userInformation = userData?.UserPersonalInformation;
          const userMember = {
            id: userData.id,
            title: userInformation.title,
        ...
        const roomsData = [];
        for (let j = 0; j < Bidding.bookingGroupData.totalRooms; j++) {
